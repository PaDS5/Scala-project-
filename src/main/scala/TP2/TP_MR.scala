...
object TP_MR {
  ...
  def nthA[T](x: Int, l: List[T]): Option[T] = if (x < 0) None else l match {
  	case head :: tail if x > 0 => nthA(x-1, tail)
  	case head :: tail if x == 0 => Some(head)
  	case Nil if x == 0 => None
  	case _ => None
  }
  ...
  def reverseA[T](list: List[T]): List[T] = list.foldLeft(List.empty[T]) { (next, curr) => curr :: next }
  ...
  def salarySum(employees: List[Employee]): Double = employees.foldLeft(0.0) { (acc, elem) => acc + elem.salary }
  ...
  def addressOf(users: List[User]): List[String] = users.foldRight(List.empty[String]) { (elem, acc) => elem.address :: acc }
  ...
  def average(values: Iterator[Double]): Option[Double] = if (values.length == 0) None else Some(values.foldLeft((0.0, 0)) { (acc, elem) => (acc._1+elem, acc._2+1) }).collect( elem => elem._1 / elem._2 )
